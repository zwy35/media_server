...
class AACTransport {
    constructor(client, stream, details) {
        ...
        this.client = client;
        this.stream = stream;
        console.log('[AACTransport][constructor]---1');
        client.on("data", (channel, data, packet) => {
            console.log('[AACTransport][constructor]---2');
            console.log('[AACTransport][constructor] channel = ', channel, ', details.rtpChannel = ', details.rtpChannel );
            if (channel == details.rtpChannel) {
                console.log('[AACTransport][constructor]---3');
                this.processRTPPacket(packet);
                console.log('[AACTransport][constructor]---4');
            }
            console.log('[AACTransport][constructor]---5');
        });
        ...
        console.log('[AACTransport][constructor]---6');
        ...
        ***/
        ...
        console.log('[AACTransport][constructor]---7');
    }
    processRTPPacket(packet) {
        ...
        console.log('[AACTransport][processRTPPacket]---1');
        let rtp_payload = packet.payload;
        let ptr = 0;
        let audio_data = [];
        console.log('[AACTransport][processRTPPacket]---2');
        while (true) {
            console.log('[AACTransport][processRTPPacket]---3');
            if (ptr + 4 > rtp_payload.length)
                break; // 2 bytes for AU Header Length, 2 bytes of AU Header payload
            console.log('[AACTransport][processRTPPacket]---4');
            ...
            ptr += aac_frame_size;
            console.log('[AACTransport][processRTPPacket]---5');
        }
        // Write Audio Data Transport Stream (adts) header
        // followed by the AAC data
        console.log('[AACTransport][processRTPPacket]---6');
        ...
        console.log('[AACTransport][processRTPPacket]---7');
    }
}
